...
// The calibration constants moved WITH the predicate that reads them, into
// `radiative_frost_decline.dart` (2026-08-06). The coverage boundary is still
// CALLED, never re-derived — the calibration package forbids a second copy of
// this threshold logic by name (cal:123-129); there remains exactly one.

// The radiative-frost decline predicate now lives in its own pure-Dart
// library so the road-surface classifier can call the SAME predicate without
// dragging in the fetch path. Re-exported so every existing import of this
// file keeps resolving `radiativeFrostJudgementDeclined` and
// `kJmaTemperatureStepCelsius` unchanged.
export 'radiative_frost_decline.dart';
import 'radiative_frost_decline.dart';
